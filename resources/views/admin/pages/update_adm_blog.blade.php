                ...
                <div class="col-lg-12 col-xlg-9 col-md-7 px-5">
                    <div class="card">
                        <div class="mt-3 px-4">
                            ...
                            <form action="{{ url('adm_blog/update/' . $adm_blog->id) }}" method="POST" enctype="multipart/form-data">
                                @csrf
                                ...
                                <div class="row input_field" style="">
                                    ...
                                    <div class="col-sm-12 col-md-6">
                                        ...
                                        <select class="form-select category" id="category_id" name="category_id" aria-label="Default select example"
                                            ...
                                            @foreach ($blog_category as $item)
                                             <option value="{{$item->id}}"{{$item->id === $adm_blog->category_id ? 'selected':''}}>{{$item->category}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="col-sm-12 col-md-6">
                                    ...
                                    <div class="col-sm-12 col-md-6">
                                        ...
                                        <input type="file" class="form-control"  value="{{$adm_blog->image}}" id="image" name="image">
                                            <img src="{{url('assets/uploads/'.$adm_blog->image)}}" width="100px" alt="">
                                    </div>
                                    <div class="col-sm-12 col-md-6">
                                        <label for="status">Status</label>
                                        <select class="form-select" name="status" aria-label="Default select example"
                                            required>
                                            <option value="Active"{{$adm_blog->status === 'Active' ? 'selected':''}}>Active</option>
                                            <option value="Deactive"{{$adm_blog->status === 'Deactive' ? 'selected':''}}>Deactive</option>
                                          </select>
                                    </div>
